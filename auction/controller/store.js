...
export async function getProducts(req, res) {
    // seller_Id가 없으면 전체상품 있으면 그 상품들
    const { seller_id } = req.query;
    const data = await (!seller_id ? storeRepository.getAll() : storeRepository.getAllBysellerid(seller_id));
...
export async function getProduct(req, res) {
    ...
    try {
        ...
        res.status(404).send({ message: `Id :(${id}) add product  not found` });
    }
}
